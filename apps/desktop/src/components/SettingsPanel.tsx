...
export function SettingsPanel({ open, packs, onClose, onReload }: { open: boolean; packs: RuntimePack[]; onClose(): void; onReload(): void }) {
  ...
  return <aside className={`settings-panel ${open ? "open" : ""}`} aria-label="설정" hidden={!open}>
    ...
    <div className="panel-body">
      <section className="settings-section"><h3>모델</h3><strong className="model-file">Qwen2.5-7B-Instruct-Q4_K_M.gguf</strong><p>llama · Q4_K_M · 4.36 GB · 최대 컨텍스트 32,768</p><div className="panel-actions"><button className="button-secondary" type="button">다른 모델 선택…</button><button className="button-secondary" type="button">모델 언로드</button></div></section>
      <section className="settings-section">
        <h3>런타임 {runtimeChanged && <span className="reload-badge">재로드 대기</span>}</h3>
      ...
      <section className="settings-section"><h3>화면</h3><SegmentedControl label="테마" value={theme} onChange={changeTheme} items={[{ value: "light", label: "라이트" }, { value: "dark", label: "다크" }, { value: "system", label: "시스템" }]} /></section>
    </div>
    <div className="panel-footer"><button className="button-primary" type="button" onClick={onReload}>적용하고 모델 다시 로드</button><p>변경한 런타임과 옵션은 모델을 다시 로드해야 적용됩니다.</p></div>
  </aside>;
}

...
interface Props {
  ...
  onModelSelect?(): void; modelSelectDisabled?: boolean; onRename?(title: string): void | Promise<void>; loadingProgress?: number | null;
}

export function ChatHeader({ title, modelName, modelState, settingsOpen, settingsButtonRef, resetButtonRef, onSettings, onReset, onModelSelect, modelSelectDisabled, onRename, loadingProgress }: Props) {
  ...
  return (
    <header className="chat-header">
      ...
      <button type="button" className={`model-chip ${modelState}`} aria-label="GGUF 모델 선택" onClick={onModelSelect} disabled={modelSelectDisabled}>

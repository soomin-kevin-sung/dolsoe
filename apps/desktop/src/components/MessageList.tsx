      ...
      {messages.map((message) => message.role === "user" ? (
        ...
        <div className="message assistant assistant-rich" data-long-message={message.id === "long" || undefined} key={message.id}>
          ...
          <div className="assistant-content">
            ...
            {message.metrics && <div className={`metrics-line ${message.status === "streaming" ? "live" : ""}`}>{message.metrics}</div>}
            {message.status !== "streaming" && message.content && <div className="message-actions"><button className="icon-button" type="button" aria-label="응답 복사" title="응답 복사" onClick={() => void navigator.clipboard.writeText(message.content)}><Copy size={14} /></button></div>}
          </div>
        </div>
      ))}

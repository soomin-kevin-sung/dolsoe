...
function EmptyContent({ state }: { state: MockStateName }) {
  if (state === "no-model") return <div className="empty-state"><Box size={28} strokeWidth={1.5} /><h2>선택된 모델이 없습니다</h2><p>로컬 GGUF 파일을 선택하면 대화를 시작할 수 있습니다. 모든 추론은 이 PC에서만 실행됩니다.</p><button className="button-primary" type="button"><FolderOpen size={14} /> GGUF 모델 선택…</button><span className="caption">지원 형식: .gguf</span></div>;
  ...
  return <div className="empty-state"><BookOpen size={28} strokeWidth={1.5} /><h2>새 대화를 시작하세요</h2><p>메시지를 입력하면 이 PC에서 바로 추론이 실행됩니다. 대화 내용은 외부로 전송되지 않습니다.</p><span className="ready-line"><span className="status-dot ready" />Qwen2.5-7B-Instruct · CUDA · 준비됨</span></div>;
}
